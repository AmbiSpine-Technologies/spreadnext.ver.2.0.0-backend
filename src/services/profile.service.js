...
import { resolveS3Folder } from "../utils/s3FolderResolver.js";
...
const updatedProfileData = cleanObject({
      ...profileData,
      personalInfo: cleanObject({
	...
	if (profile) {
      Object.keys(updatedProfileData).forEach((key) => {
        if (key === "personalInfo") {
          Object.keys(updatedProfileData.personalInfo).forEach((field) => {
            profile.personalInfo[field] =
              updatedProfileData.personalInfo[field];
          });
      ...
      try {
        await profile.save();
        console.log("✅ Profile updated successfully");      } catch (saveError) {
  ...
  try {
    ...
    if (files?.profileImage?.length) {
      ...
      const uploaded = await uploadToS3(
        file,
        "profiles/images/profile-image"
      );

      updateData["personalInfo.profileImage"] = {
        key: uploaded.key,
        url: uploaded.url,
      };
    }

    if (files?.profileCover?.length) {
      const file = files.profileCover[0];

      const uploaded = await uploadToS3(
        file,
        "profiles/images/profile-cover"
      );

      updateData["personalInfo.profileCover"] = {
        key: uploaded.key,
        url: uploaded.url,
      };
    }
    ...
    if (Object.keys(updateData).length === 0) {
      return {
        success: false,
        message: "No media fields provided",
      };
    }
